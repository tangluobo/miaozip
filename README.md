...
- 解压上述七种格式及 RAR；另支持单文件 GZ/BZ2/XZ/ZST 与 TGZ/TBZ/TBZ2/TXZ/TZST 别名。RAR 和单文件压缩格式目前仅支持解压，不支持创建；暂不支持 CAB 等未列出的格式
- 打开已有 ZIP 后可用“添加”按钮或把文件、文件夹拖入列表继续添加；包内选中条目可通过工具栏或右键菜单删除。更新过程先写同目录临时 ZIP，成功后才替换原文件
- ZIP 支持 AES-256 密码创建、解锁、修改、移除、解压和包内文件预览；密码仅保留在当前运行会话，不写入设置文件
- 打开 ZIP 后可生成自解压程序；自解压文件包含当前平台的妙压程序与原 ZIP，运行后直接进入解压流程，加密 ZIP 会要求密码
...
主界面会显示当前系统的真实磁盘容量和可用空间；因此数值与参考截图不一定相同。ZIP 编辑、密码和自解压已实现；7z、RAR、TAR 等格式仍按各自能力只读浏览或重新创建，不会伪装成可原地编辑。7z 的“速度”选项暂不调整 LZMA2 压缩级别。
...
Windows 系统集成入口位于“工具箱”或标题栏右上角菜单，设置窗口分为“综合”和“关联”两页。交互安装完成后会启动妙压并显示默认应用提示；确认后仅为当前用户更改没有 Windows UserChoice 的格式。已有 UserChoice 时会自动打开 Windows 默认应用页面供用户确认，程序不会伪造或篡改 UserChoice。成功确认后不再重复提示。右键菜单只写入当前用户的注册表，不需要管理员权限；移动或重命名可执行文件后，可在“综合”页重新注册或修复。“添加到 ZIP”支持多选文件或文件夹，并直接打开一个独立压缩窗口，不弹出主界面；压缩进度与结果也显示在这个窗口。解压和挂载菜单仍只支持单项选择。Windows 11 的经典菜单项通常显示在“显示更多选项”中。双击或通过默认应用打开压缩文件时，仅显示主窗口中的包内列表；选择工具栏“解压到”才进入解压对话框。右键“使用妙压解压”仍直接进入解压对话框；右键打开 ISO 会进入虚拟光驱对话框，确认后才挂载。复合扩展名的 TAR.GZ 等文件可在应用内解压；Windows 根据最后一个扩展名关联 `.tar.gz` 为 `.gz`。注册覆盖 `.zip`、`.7z`、`.rar`、`.tar`、`.gz`、`.bz2`、`.xz`、`.zst`、`.tgz`、`.tbz`、`.tbz2`、`.txz`、`.tzst`，ISO 仅注册右键挂载。
